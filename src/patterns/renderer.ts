...
export function getPatternSvgString(patternIndex: number, x: number, y: number): string {
  ...
  switch (patternIndex) {
    ...
    case 2: // P3: Diagonal Cross
      foreground =
        svgLine(x, y, x + 1, y + 1, 'white', strokeWidth) +
    ...
    case 5: // P6: Corner Dots (using squares for simplicity)
      foreground =
        svgRect(x + dotOffset, y + dotOffset, dotSize, dotSize, 'white') + // TL
        ...
        svgRect(
          x + 1 - dotOffset - dotSize,
          y + 1 - dotOffset - dotSize,
          dotSize,
          dotSize,
          'white'
        ); // BR
    ...
    case 6: // P7: Checkered
      foreground =
        svgRect(x, y, 0.5, 0.5, 'white') + // TL quadrant
